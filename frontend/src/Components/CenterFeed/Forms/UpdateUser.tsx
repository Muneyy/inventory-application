...
function UpdateUser() {
        ...
        const fetchData = async () => {
            try {
                const res = await axios.get(
                    "http://localhost:3000/users/handles"
                );
                setUsersHandles(
                    res.data
                        .filter(
                            (user: any) =>
                                ...
                                user.handle !== loggedinUser.handle
                        )
    ...
    const SignupSchema = Yup.object().shape({
        ...
        handle: Yup.string()
            ...
            .matches(
                /^[a-zA-Z0-9]+$/,
                "Handle must not contain spaces or special characters"
            )
            .test(
                "handle-not-taken",
            ...
            <form onSubmit={formik.handleSubmit}>
                ...
                <FormControl isRequired>
                    ...
                    {formik.errors.handle && formik.touched.handle ? (
                        ...
                        <FormHelperText>
                            Handle must be 6-12 characters and unique.
                        </FormHelperText>
                    )}
                </FormControl>
                <Divider my="1rem" />
                ...
                <Button disabled={submitting} type="submit" colorScheme="teal">
                    {submitting ? <Spinner></Spinner> : "Update"}
                </Button>
                {/* </Formik> */}
            </form>
        </Box>
